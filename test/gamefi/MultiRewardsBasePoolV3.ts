...
describe("BasePool - MultiRewardsV3", function () {
    ...
    describe("updateEscrowPortion", async() => {
        it("Should fail when not admin (deployer)", async() => {
            await expect(basePool.updateEscrowPortions(rewardToken1.address, "100000000000000000")).to.be.revertedWith("MultiRewardsBasePoolV3: only admin");
        });
        ...
        it("Should not update escrowPortion when token are not already in the reward list", async() => {
            await expect(basePool.connect(deployer).updateEscrowPortions(rewardToken3.address,"100000000000000000"))
                .to.be.revertedWith("MultiRewardsBasePoolV3.updateEscrowPortions: reward token not in the list");
        });
        ...
        it("Should not update escrowPortion when escrowPortion is greater than 1e18", async() => {
            await expect(basePool.connect(deployer).updateEscrowPortions(rewardToken1.address, "10000000000000000000"))
                .to.be.revertedWith("MultiRewardsBasePoolV3.updateEscrowPortions: cannot escrow more than 100%");
        });
    ...
    describe("claimRewards - multiple with blacklist", async() => {
        ...
        it("blacklist - 3 player case", async() => {
            await basePool.connect(deployer).updateEscrowPortions(rewardToken1.address, parseEther("0.5"));
            await basePool.connect(deployer).updateEscrowPortions(rewardToken2.address, parseEther("0.5"));
