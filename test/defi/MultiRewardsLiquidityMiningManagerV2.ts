...
describe("LiquidityMiningManager - MultiRewards", function () {
    ...
    const pools: MultiRewardsTimeLockNonTransferablePoolV2[] = [];
    let escrowPool1: TimeLockNonTransferablePool;
    let escrowPool2: TimeLockNonTransferablePool;

    let liquidityMiningManager1: MultiRewardsLiquidityMiningManagerV2;
    let liquidityMiningManager2: MultiRewardsLiquidityMiningManagerV2;
    ...
    before(async() => {
        ...
        liquidityMiningManager1 = await (new MultiRewardsLiquidityMiningManagerV2__factory(deployer)).deploy(rewardToken1.address, rewardSource.address, DISTRIBUTOR_INCENTIVE, PLATFORM_FEE, treasury.address);
        liquidityMiningManager2 = await (new MultiRewardsLiquidityMiningManagerV2__factory(deployer)).deploy(rewardToken2.address, rewardSource.address, DISTRIBUTOR_INCENTIVE, PLATFORM_FEE, treasury.address);
